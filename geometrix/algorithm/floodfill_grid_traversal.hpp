...
namespace geometrix
{
    ...
    public:
        ...
        static const std::size_t BOUNDARY = (std::numeric_limits<std::size_t>::max)();
        ...
        void traversal(const Point& p, Visitor&& visitor)
        {
            traversal(m_gridTraits.get_x_index(get<0>(p), m_gridTRaits.get_y_index(get<1>(p)), std::forward<Visitor>(v));
        }

        template <typename Visitor>
        void traversal(std::uint32_t x, std::uint32_t y, Visitor&& v)
        {
        ...
        template <typename PointSequence, typename std::enable_if<is_polyline<PointSequence>::value, int>::type = 0>
        void mark_boundary( const PointSequence& pline )
        {
            typedef point_sequence_traits<PointSequence>::point_type point_type;
            typedef segment<point_type> segment_type;
            auto visitor = [this](std::uint32_t i, std::uint_32_t j){ get_color(i, j) = BOUNDARY; }
        ...
        template <typename PointSequence, typename std::enable_if<is_polygon<PointSequence>::value, int>::type = 0>
        void mark_boundary(const PointSequence& pgon)
        {
            typedef point_sequence_traits< PointSequence >::point_type point_type;
            typedef segment< point_type > segment_type;
            auto visitor = [this](std::uint32_t i, std::uint_32_t j){ get_color(i, j) = BOUNDARY; }
        ...
        template <typename Segment, typename std::enable_if<is_segment<Segment>::value, int>::type = 0>
        void mark_boundary( const Segment& segment )
        {
            auto visitor = [this](std::uint32_t i, std::uint_32_t j){ get_color(i, j) = BOUNDARY; }
            fast_voxel_grid_traversal( segment, visitor );
        }
