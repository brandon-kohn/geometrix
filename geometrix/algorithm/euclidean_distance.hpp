...
//  template <typename Point, typename Polyline>
//  inline typename result_of::point_polyline_distance_sqrd<Point, Polyline>::type point_polyline_distance_sqrd_old(const Point& p, const Polyline& poly)
//  {
//      using access = point_sequence_traits<Polyline>;
//      using segment_type = segment<typename geometric_traits<Polyline>::point_type>;
//      using distance_sqrd_type = typename result_of::point_segment_distance_sqrd<Point, segment_type>::type;
...
//      auto size = access::size(poly);
//      for (std::size_t i = 0, j = 1; j < size; i = j++)
//          distance = (std::min)(distance, point_segment_distance_sqrd(p, access::get_point(poly, i), access::get_point(poly, j)));
// 
//      return distance;
//  }
    ...
    inline typename result_of::point_polyline_distance_sqrd<Point, Polyline>::type point_polyline_distance_sqrd(const Point& p, const Polyline& poly)
    {
        ...
        point_t b = access::get_point(poly, 0);
        vector_t pb = b - p;
        auto dist = magnitude_sqrd(pb);
        for (size_t i = 1; i < size; ++i)
        {
            point_t a = b;
            point_t pa = pb;
            b = access::get_point(poly, i);
            pb = b - p;
            vector_t ba = a - b;
            auto t = dot_product(ba, pa);
            if (t >= constants::zero<decltype(t)>())
            {
                auto distBA = magnitude_sqrd(ba);
                area_t d = (t <= distBA) ? square(exterior_product_area(ba, pb)) / distBA : magnitude_sqrd(pb);
                //area_t d = (t > distBA) ? magnitude_sqrd(pb) : square(exterior_product_area(ba, pb)) / distBA;
                if (d < dist)
                    dist = d;
            }
        }
    ...
    namespace result_of {
        ...
        {
            using segment_type = segment<typename geometric_traits<Polyline>::point_type>;
            using type = typename result_of::segment_segment_distance_sqrd<Segment, segment_type>::type;
        };
    }
    ...
    inline typename result_of::segment_polyline_distance_sqrd<Segment, Polyline>::type segment_polyline_distance_sqrd(const Segment& seg, const Polyline& poly, const NumberComparisonPolicy& cmp)
    {
        using access = point_sequence_traits<Polyline>;
        using segment_type = segment<typename geometric_traits<Polyline>::point_type>;
        using distance_sqrd_type = typename result_of::segment_segment_distance_sqrd<Segment, segment_type>::type;
        auto distance = std::numeric_limits<distance_sqrd_type>::infinity();
        auto size = access::size(poly);
        for (std::size_t i = 0, j = 1; j < size; ++i, ++j)
            distance = (std::min)(distance, segment_segment_distance_sqrd(seg, make_segment(access::get_point(poly, i), access::get_point(poly, j)), cmp));
    ...
    namespace result_of {
        ...
        {
            using segment_type = segment<typename geometric_traits<Polyline>::point_type>;
            using type = typename result_of::segment_segment_distance<Segment, segment_type>::type;
        };
    }
