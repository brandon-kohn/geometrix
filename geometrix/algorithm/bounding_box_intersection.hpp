...
namespace geometrix {
    ...
    public:
        ...
        orthogonal_range( const PointSequence& pointSequence, const NumberComparisonPolicy& compare )
        {
            ...
            typedef typename point_traits< point_type >::arithmetic_type            coordinate_type;
            ...
            boost::array< coordinate_type, dimension_of< point_type >::type::value > low;
            tensor_traversal::for_each( *pIt, assign_to_array< boost::array< coordinate_type, dimension_of< point_type >::type::value > >( low ) );
            boost::array< coordinate_type, dimension_of< point_type >::type::value > high = low;
            
            //! check bounds against each point in the sequence updating the limits on each dimension.
            while( pIt != pEnd )
            {
                const point_type& p = *pIt++;
                tensor_traversal::for_each( p,
    ...
    private:
        ...
        template <typename NumericSequence>
        struct comparer<NumericSequence, typename boost::enable_if< is_compile_time< NumericSequence > >::type >
        {
        ...
        template <typename NumericSequence>
        struct comparer<NumericSequence, typename boost::enable_if< is_run_time< NumericSequence > >::type >
        {
            template <typename NumberComparisonPolicy>
            static bool compare( const NumericSequence& n, const NumericSequence& low, const NumericSequence& high, const NumberComparisonPolicy& nCompare )
            {
                for( std::size_t i=0;i < dimension_of<NumericSequence>::type::value; ++i )
                {
                    if( nCompare.less_than( get( n, i ), get( low, i ) ) ||
                        nCompare.greater_than( get( n, i ), get( high, i ) ) )                    
                    return false;
                }
