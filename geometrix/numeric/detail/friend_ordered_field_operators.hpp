...
#ifdef GEOMETRIX_TRY_CONSTEXPR
#include <boost/config.hpp>
#define GEOMETRIX_CONSTEXPR BOOST_CONSTEXPR
#else
#define GEOMETRIX_CONSTEXPR 
#endif

//! Comparison
#define GEOMETRIX_FRIEND_LESS_THAN_COMPARABLE_OPERATORS(T, U)                                            \
GEOMETRIX_CONSTEXPR friend bool operator<=(const T& x, const U& y) { return !static_cast<bool>(x > y); } \
GEOMETRIX_CONSTEXPR friend bool operator>=(const T& x, const U& y) { return !static_cast<bool>(x < y); } \
GEOMETRIX_CONSTEXPR friend bool operator>(const U& x, const T& y)  { return y < x; }                     \
GEOMETRIX_CONSTEXPR friend bool operator<(const U& x, const T& y)  { return y > x; }                     \
GEOMETRIX_CONSTEXPR friend bool operator<=(const U& x, const T& y) { return !static_cast<bool>(y < x); } \
GEOMETRIX_CONSTEXPR friend bool operator>=(const U& x, const T& y) { return !static_cast<bool>(y > x); } \
/***/

#define GEOMETRIX_FRIEND_LESS_THAN_COMPARABLE_SELF_OPERATORS(T)                                          \
GEOMETRIX_CONSTEXPR friend bool operator>(const T& x, const T& y)  { return y < x; }                     \
GEOMETRIX_CONSTEXPR friend bool operator<=(const T& x, const T& y) { return !static_cast<bool>(y < x); } \
GEOMETRIX_CONSTEXPR friend bool operator>=(const T& x, const T& y) { return !static_cast<bool>(x < y); } \
/***/
...
#define GEOMETRIX_FRIEND_EQUALITY_COMPARABLE_SELF_OPERATORS(T)           \
    GEOMETRIX_CONSTEXPR friend bool operator!=(const T& x, const T& y)   \
    { return !static_cast<bool>(x == y); }                               \
/***/

#define GEOMETRIX_FRIEND_INCREMENTABLE_OPERATOR(T)                       \
    GEOMETRIX_CONSTEXPR friend T operator++(T& x, int)                   \
    {                                                                    \
        T nrv(x);                                                        \
        ++x;                                                             \
        return nrv;                                                      \
...
#define GEOMETRIX_FRIEND_BINARY_OPERATOR_COMMUTATIVE( T, U, OP )                             \
    GEOMETRIX_CONSTEXPR friend T operator OP( T lhs, const U& rhs ) { return lhs OP##= rhs; }\
    GEOMETRIX_CONSTEXPR friend T operator OP( const U& lhs, T rhs ) { return rhs OP##= lhs; }\
/***/

#define GEOMETRIX_FRIEND_BINARY_OPERATOR_COMMUTATIVE_SELF( T, OP )                           \
    GEOMETRIX_CONSTEXPR friend T operator OP( T lhs, const T& rhs ) { return lhs OP##= rhs; }\
/***/

#define GEOMETRIX_FRIEND_BINARY_OPERATOR_NON_COMMUTATIVE( T, U, OP )                         \
    GEOMETRIX_CONSTEXPR friend T operator OP( T lhs, const U& rhs ) { return lhs OP##= rhs; }\
    GEOMETRIX_CONSTEXPR friend T operator OP( const U& lhs, const T& rhs )                   \
    { return T( lhs ) OP##= rhs; }                                                           \
/***/
