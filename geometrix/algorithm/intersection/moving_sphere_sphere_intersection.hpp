...
namespace geometrix {
	...
	template <typename Sphere, typename Vector, typename NumberComparisonPolicy>
	inline bool moving_sphere_sphere_intersection(const Sphere& s0, const Sphere& s1, const Vector& v0, const Vector& v1, const NumberComparisonPolicy& cmp)
	{
		using length_t = typename arithmetic_type_of<Vector>::type;
		using point_t = point<length_t, dimension_of<Vector>::value>;
