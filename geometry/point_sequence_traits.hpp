...
//
#ifndef GENERATIVE_GEOMETRY_POINT_SEQUENCE_TRAITS_HPP
...
{                                                                      \
    ...
    static const_reverse_iterator                rend( const container_type& p ) { return p.rend(); }     \
 	static size_t                                size( const container_type& p ) { return p.size(); }     \
    static bool                                  empty( const container_type& p ) { return p.empty(); }   \
    static const point_type&                     get_point( const container_type& pointSequence, size_t index ) { const_iterator iter( pointSequence.begin() ); std::advance( iter, index ); return *iter; } \
    static point_type&                           get_point( container_type& pointSequence, size_t index ) { iterator iter( pointSequence.begin() ); std::advance( iter, index ); return *iter; }       \
    ...
    static boost::shared_ptr< container_type >   construct( InputIterator first, InputIterator last ){ boost::shared_ptr< container_type > p( new container_type( first, last ) ); return p; } \
};
...
{                                                                                \
    ...
    static size_t                                size( const container_type& p ) { return p.size(); }    \
    static bool                                  empty( const container_type& p ) { return p.empty(); }  \
    static const point_type&                     get_point( const container_type& pointSequence, size_t index ) { return pointSequence[index]; } \
    static point_type&                           get_point( container_type& pointSequence, size_t index ) { return pointSequence[index]; }       \
...
#endif //GENERATIVE_GEOMETRY_POINT_SEQUENCE_TRAITS_HPP
