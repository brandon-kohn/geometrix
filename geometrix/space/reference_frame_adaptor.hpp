...
template <typename T>
struct is_reference_frame_adaptor : boost::false_type {};
...
struct is_reference_frame_adaptor< reference_frame_adaptor<T,F> > : boost::true_type {};

//! setup traits for base types
template <typename Sequence, typename ReferenceFrame>
struct geometric_traits< reference_frame_adaptor< Sequence, ReferenceFrame > > : geometric_traits< Sequence >
{
    typedef ReferenceFrame reference_frame;
};

template <typename Sequence, typename ReferenceFrame>
struct tensor_traits< reference_frame_adaptor< Sequence, ReferenceFrame > > : tensor_traits< typename remove_const_ref<Sequence>::type >{};
