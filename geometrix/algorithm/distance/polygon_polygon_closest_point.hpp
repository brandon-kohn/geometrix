...
//
#ifndef GEOMETRIX_ALGORITHM_POLYGON_POLYGON_CLOSEST_POINT_HPP
#define GEOMETRIX_ALGORITHM_POLYGON_POLYGON_CLOSEST_POINT_HPP
#pragma once

#include <geometrix/algorithm/distance/segment_polygon_closest_point.hpp>
...
namespace geometrix {
    namespace result_of {
        ...
        struct polygon_polygon_closest_point
        {
		private:
            using point1_t = typename geometric_traits<Polygon1>::point_type;
            using point2_t = typename geometric_traits<Polygon2>::point_type;
		public:
    ...
    template <typename Polygon1, typename Polygon2, typename Dimensionless, typename Point, typename NumberComparisonPolicy>
    inline typename result_of::polygon_polygon_closest_point<Polygon1, Polygon2>::type polygon_polygon_closest_point_brute(const Polygon1& p1, const Polygon2& p2, std::size_t& i1, std::size_t& i2, Dimensionless& s, Dimensionless& t, Point& c1, Point&c2, const NumberComparisonPolicy& cmp)
    {
        ...
        for (std::size_t i = size-1, j = 0; j < size; i = j++)
        {
            ...
            Point c1l, c2l;
			std::size_t i2l;
            auto d2 = segment_polygon_closest_point(access1::get_point(p1, i), access1::get_point(p1, j), p2, i2l, sl, tl, c1l, c2l, cmp);
            if(d2 < minDist2)
            {
                minDist2 = d2;
				i1 = i;
    ...
    template <typename Polygon1, typename Polygon2, typename Dimensionless, typename Point, typename NumberComparisonPolicy>
    inline typename result_of::polygon_polygon_closest_point<Polygon1, Polygon2>::type polygon_polygon_closest_point_brute(std::tuple<std::size_t, std::size_t> const& subRange1, const Polygon1& p1, std::tuple<std::size_t, std::size_t> const& subRange2, const Polygon2& p2, std::size_t& i1, std::size_t& i2, Dimensionless& s, Dimensionless& t, Point& c1, Point&c2, const NumberComparisonPolicy& cmp)
    {
        using access1 = point_sequence_traits<Polygon1>;
        using point_t = typename point_sequence_traits<Polygon1>::point_type;
        using length_t = typename geometric_traits<point_t>::arithmetic_type;
        ...
        for (std::size_t i = std::get<0>(subRange1), j = std::get<0>(subRange1) + 1; j <= std::get<1>(subRange1); i = j++)
        {
            ...
            Point c1l, c2l;
			std::size_t i2l;
            auto d2 = segment_polygon_closest_point(access1::get_point(p1, i), access1::get_point(p1, j), subRange2, p2, i2l, sl, tl, c1l, c2l, cmp);
            if(d2 < minDist2)
            {
                minDist2 = d2;
				i1 = i;
    ...
    template <typename Polygon1, typename Polygon2, typename Dimensionless, typename Point, typename NumberComparisonPolicy>
    inline typename result_of::polygon_polygon_closest_point<Polygon1, Polygon2>::type polygon_polygon_closest_point(const Polygon1& p1,  const Polygon2& p2, std::size_t& i1, std::size_t& i2, Dimensionless& s, Dimensionless& t, Point& c1, Point& c2, const NumberComparisonPolicy& cmp)
    {
        using access1 = point_sequence_traits<Polygon1>;
        using access2 = point_sequence_traits<Polygon2>;
        if(access1::size(p1) < 6 || access2::size(p2) < 6)
            return polygon_polygon_closest_point_brute(p1, p2, i1, i2, s, t, c1, c2, cmp);
        ...
        while(!Q.empty())
        {
            ...
            if ((rh1 - rl1) < 5 || (rh2 -rl2) < 5)
            {
                ...
                Point c1l, c2l;
				std::size_t i1l, i2l;
                auto dl = polygon_polygon_closest_point_brute(r1, p1, r2, p2, i1l, i2l, sl, tl, c1l, c2l, cmp);
                if(dl < minDist2)
                {
                    minDist2 = dl;
					i1 = i1l;
...
#endif //GEOMETRIX_ALGORITHM_POLYGON_POLYGON_CLOSEST_POINT_HPP
