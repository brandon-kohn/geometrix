...
{                                                                         
    ...
    //! \brief compile time access if available for the sequence.
    template <unsigned int Index>
    static typename cartesian_frame::coordinate_type_of<sequence_type, Index>::type get( const sequence_type& sequence ) 
    {   
        typedef typename cartesian_frame::coordinate_type_of<sequence_type, Index>::type coordinate_type;
        typedef typename cartesian_frame::unit_at<Index>::type                           unit_type;
...
typename typename cartesian_reference_frame< dimension_of<Tensor>::value >::coordinate_type_of< Tensor, e_x >::type get_x( const Tensor& seq )
{
    return cartesian_access_traits< Tensor >::get<e_x>( seq );
}
template <typename Tensor>
typename typename cartesian_reference_frame< dimension_of<Tensor>::value >::coordinate_type_of< Tensor, e_y >::type get_y( const Tensor& seq )
{
    return cartesian_access_traits< Tensor >::get<e_y>( seq );
}
template <typename Tensor>
typename typename cartesian_reference_frame< dimension_of<Tensor>::value >::coordinate_type_of< Tensor, e_z >::type get_z( const Tensor& seq )
{
