...
BOOST_AUTO_TEST_CASE(SortingTestsNonZeroReferenceVector)
{
	using namespace geometrix;
	...
	absolute_tolerance_comparison_policy<double> cmp(1e-10);
	vector_angle_compare<absolute_tolerance_comparison_policy<double>> angcmp(vector_double_2d{ 0, 1 }, cmp);
	std::set < vector_2d, vector_angle_compare<absolute_tolerance_comparison_policy<double>>> sorted(vecs.begin(), vecs.end(), angcmp);
