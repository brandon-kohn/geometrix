...
namespace geometrix {
	...
	inline bool intersect_moving_sphere_plane(const Sphere& s, const Vector& velocity, const Plane& p, ArithmeticType &t, Point& q, const NumberComparisonPolicy& cmp)
	{
		...
		ArithmeticType dist = dot_product(p.get_n(), s.get_center()) - p.get_d(); 
		if( cmp.less_than_or_equal(math::abs( dist ), s.get_radius()) )
		{
			// The sphere is already overlapping the plane. Set time of 
			// intersection to zero and q to sphere center 
			t = 0;
			assign( q, s.get_center() );
			return true;
		}
		else 
		{
			ArithmeticType denom = dot_product( p.get_n(), velocity ); 
			...
			else 
			{
				...
				ArithmeticType r = dist > 0 ? s.get_radius() : -s.get_radius();
				t = (r - dist) / denom;
				assign(q, s.get_center() + t * velocity - r * p.get_n());
				return true;
			}
		}
	}
